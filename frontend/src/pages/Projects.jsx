...
const Projects = () => {
  // CRITICAL FIX: Explicitly default states to empty arrays instead of undefined
  const [projects, setProjects] = useState([]);
  const [filtered, setFiltered] = useState([]);
  const [search, setSearch] = useState('');
  const [category, setCategory] = useState('all');
  ...
  useEffect(() => {
    projectAPI.getAll()
      .then(r => {
        // Fallback checks to ensure we always commit an array to state
        const projectData = r?.data?.data || r?.data || [];
        setProjects(Array.isArray(projectData) ? projectData : []);
        setFiltered(Array.isArray(projectData) ? projectData : []);
      })
      .catch((err) => {
        console.error("API Fetch Error:", err);
        // CRITICAL FIX: If API fails, reset states to empty arrays so .filter doesn't crash
        setProjects([]);
        setFiltered([]);
      })
  ...
  useEffect(() => {
    // CRITICAL FIX: Force fallback to an empty array if projects is somehow falsy
    let result = projects || [];
    ...
    if (search) {
      result = result.filter(p =>
        p && (
          ...
          (p.technologies && Array.isArray(p.technologies) && p.technologies.some(t => t.toLowerCase().includes(search.toLowerCase())))
        )
      );
    }
    setFiltered(result);
              ...
              project && project._id && (
                <motion.div key={project._id} layout initial={{ opacity: 0, y: 30 }}
                  ...
                  <div className="p-5">
                    ...
                    <p className="text-dark-500 dark:text-dark-400 text-sm mb-4 line-clamp-2">{project.description}</p>
                    <div className="flex flex-wrap gap-1.5">
                      {project.technologies && Array.isArray(project.technologies) && project.technologies.slice(0, 4).map(t => (
                        <span key={t} className="text-xs px-2 py-1 rounded-md bg-dark-100 dark:bg-dark-800 text-dark-600 dark:text-dark-400">{t}</span>
                      ))}
                      {project.technologies && Array.isArray(project.technologies) && project.technologies.length > 4 && <span className="text-xs px-2 py-1 rounded-md bg-dark-100 dark:bg-dark-800 text-dark-500">+{project.technologies.length - 4}</span>}
                    </div>
                  </div>
                </motion.div>
              )
