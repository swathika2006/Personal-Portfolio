...
// REMOVED: Cleaned out all "proficiency" keys from fallback data
const fallbackSkills = [
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {group.skills.map((skill, i) => (
                  <motion.div key={skill.name} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }}
                    ...
                    {/* CHANGED: Removed margins and updated flex alignment to perfectly center elements horizontally & vertically */}
                    <div className="flex items-center gap-4 py-1">
                      <span className="text-3xl text-dark-700 dark:text-dark-200">{iconMap[skill.icon] || <FaReact />}</span>
                      <div className="flex-1">
                        <h4 className="font-semibold text-base text-dark-900 dark:text-white">{skill.name}</h4>
                      </div>
                    </div>

                    {/* REMOVED: The percentage text indicator and the entire skill-bar HTML wrapper have been completely dropped */}
